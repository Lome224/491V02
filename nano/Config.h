...
// ---------------------------------------------------------------------------
inline constexpr unsigned long kVl53l0xTimeoutMs      = 60;
inline constexpr uint16_t      kVl53l0xSamplePeriodMs = 0;     // 0 = as fast as the sensor allows
inline constexpr uint16_t      kVl53l0xMaxDistanceMm  = 1200;  // readings above this are rejected

// Min-distance detection filtering. Kept short so the motor reacts quickly,
// but long enough that a single noisy sample doesn't trip a pause.
inline constexpr uint8_t  kVl53l0xDetectHistoryWindow = 9;    // odd number is nicer for median
...
inline constexpr uint16_t kVl53l0xTrendDeltaMm        = 6;    // mm of margin for turn-around detection
inline constexpr uint16_t kVl53l0xMinStableTimeMs     = 100;  // closer/away must persist this long
inline constexpr uint8_t  kVl53l0xRecoveryTimeouts    = 20;   // re-init after N consecutive timeouts
